...
*Nota:* Este comando solo podrá ser usado por owners del Bot.`.trim();
  ...
  const isEnable = /تفعيل|true|enable|on|1/i.test(command);
  ...
  conn.sendMessage(m.chat, {text: `🗂️ *الخيار*: *${type}*\n🎚️ *الحاله:* *${isEnable ? 'مفعل' : 'معطل'}*\n📣 *الوضعيه*: *${isAll ? 'للبوت فقط' : isUser ? '' : 'للمجموعات فقط'}*`}, {quoted: m});
};
handler.help = ['en', 'dis'].map((v) => v + 'able <option>');
handler.tags = ['group', 'owner'];
handler.command = /^تفعيل|((en|dis)able|(tru|fals)e|(turn)?[01])$/i;
