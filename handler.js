...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `مرحبا *@${nk.from.split('@')[0]}*ال ${nk.isVideo ? 'مكالمة الفيديو' : 'مكالمة'} غير مسموح بها من قبل المطور،لذلك تم حظرك.\n-\n لذلك نوصيك بالتكلم مع المطور ليمسح الحظر،لكن رجاءً حتى لو المطور مسح الحظر لا تتصل بالبوت مرره اخرى كي لا يفعل لك حظر مره اخرى!`, true, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝐈𝐓𝐀𝐂𝐇𝐈┇👑;;;\nFN:𝐈𝐓𝐀𝐂𝐇𝐈┇👑\nORG:𝐈𝐓𝐀𝐂𝐇𝐈┇👑\nTITLE:\nitem1.TEL;waid=96178965440:+96178965440\nitem1.X-ABLabel:𝐈𝐓𝐀𝐂𝐇𝐈┇👑\nX-WA-BIZ-DESCRIPTION:[❗] تكلم مع المطور اذا كان لديك مشكله ما.+96178965440\nX-WA-BIZ-NAME:𝐈𝐓𝐀𝐂𝐇𝐈┇👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '𝐈𝐓𝐀𝐂𝐇𝐈┇👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        group: '*[ ⚠️ ملاحظه ⚠️ ] هذا الامر لمشرفي الجموعه فقط*',
        private: '*[ ⚠️ ملاحظه ⚠️ ] هذا الامر للمطور فقط*',
        admin: '*[ ⚠️ ملاحظه ⚠️ ] هذا الامر لمشرفي الجموعه فقط*',
        botAdmin: '*[ ⚠️ ملاحظه ⚠️ ] لاستخدام هذا الامر عليك رفع البوت مشرفا اولاً*',
        unreg: '*[ 🛑 𝐇𝐄𝐘!! 𝐀𝐋𝐓𝐎, 𝐍𝐎 𝐄𝐒𝐓𝐀𝐒 𝐑𝐄𝐆𝐈𝐒𝐓𝐑𝐀𝐃𝐎 🛑 ]*\n\n*—◉ 𝙿𝙰𝚁𝙰 𝚄𝚂𝙰𝚁 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙳𝙴𝙱𝙴𝚂 𝚁𝙴𝙶𝙸𝚂𝚃𝚁𝙰𝚁𝚃𝙴, 𝚄𝚂𝙰 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾*\n*➣ #verificar nombre.edad*',
        restrict: '*[ ⚠️ ملاحظه ⚠️ ] يجب على المطور تفعيل الامر لاستخدامه*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: 'ملاحظه / تنبيه', body: '『👑┇𝚁𝙰𝙸𝙻𝚈┇🤖┇𝙱𝙾𝚃┇👑』', thumbnail: imagen1, sourceUrl: 'https://wa.me/+96178965440' }}}}, aa)
